...
YaaJuu es una sola marca hacia el consumidor. Si cada asociado inventa su propio catálogo, se duplican SKUs, fotos, analítica y precios. Al mismo tiempo, cada dark store decide qué vende.

## Decisión

YaaJuu posee el catálogo maestro (`Category`, `GlobalProduct`). Las stores optan a esos productos mediante `StoreProduct` (`IsEnabled`).
