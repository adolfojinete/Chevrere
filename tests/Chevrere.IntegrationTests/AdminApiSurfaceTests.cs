...
public sealed class AdminApiSurfaceTests(ChevrereApiFactory factory)
{
    [Fact]
    public async Task Login_rejects_invalid_credentials_and_payload()
    {
        using var client = factory.CreateClientUnredirected();
        var invalid = await client.PostAsJsonAsync("/api/v1/auth/login", new LoginRequest("nobody@chevrere.test", "WrongPass!23456"));
