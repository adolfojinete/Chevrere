...
public sealed class OrdersReservationAtomicityTests(YaaJuuApiFactory factory)
{
    [Fact]
    ...
    private static YaaJuuDbContext Db(AsyncServiceScope scope) =>
        scope.ServiceProvider.GetRequiredService<YaaJuuDbContext>();
    ...
    private static void AssertNoTrackedRelease(YaaJuuDbContext db)
    {
        Assert.Equal(0, db.ChangeTracker.Entries<InventoryMovement>().Count(e => e.State == EntityState.Added));
    ...
    private static void AssertActiveTrackedReservations(YaaJuuDbContext db)
    {
        var tracked = db.ChangeTracker.Entries<InventoryReservation>().Select(e => e.Entity).ToList();
    ...
    private static async Task<BalanceSnapshot> SnapshotAsync(YaaJuuDbContext db, SeededOrder ctx)
    {
    ...
    private static async Task AssertUnchangedAsync(YaaJuuDbContext db, SeededOrder ctx, BalanceSnapshot before)
    {
