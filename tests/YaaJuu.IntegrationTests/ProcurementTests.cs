...
public sealed class ProcurementTests(YaaJuuApiFactory factory)
{
    ...
    private async Task<int> CountAsync(Func<YaaJuuDbContext, Task<int>> count)
    {
        await using var scope = factory.Services.CreateAsyncScope();
        scope.ServiceProvider.GetRequiredService<ITenantFilterBypass>().Enabled = true;
        return await count(scope.ServiceProvider.GetRequiredService<YaaJuuDbContext>());
    }
