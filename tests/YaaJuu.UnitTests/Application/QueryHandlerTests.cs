...
public sealed class QueryHandlerTests
{
    ...
    {
        var auth = Substitute.For<YaaJuu.Modules.Identity.Application.Abstractions.IUserAuthenticator>();
        var response = new YaaJuu.Modules.Identity.Application.Contracts.LoginResponse("t", FixedClock.Now, Guid.CreateVersion7(), "a@b.com", null, ["PlatformAdmin"]);
        auth.AuthenticateAsync(Arg.Any<YaaJuu.Modules.Identity.Application.Contracts.LoginRequest>(), Arg.Any<CancellationToken>())
            .Returns(Result.Success(response));
        var result = await new YaaJuu.Modules.Identity.Application.Login.LoginHandler(auth)
            .HandleAsync(new YaaJuu.Modules.Identity.Application.Contracts.LoginRequest("a@b.com", "x"), CancellationToken.None);
